...
*/
        ...
        class OwnTeamInLeagueInSeason
        {
                ...
                function OwnTeamInLeagueInSeason()
                {
                }
        ...
        class OwnTeamInLeagueInSeasonList
        {
                var $list;
                var $number;
                var $index;
                ...
                function OwnTeamInLeagueInSeasonList($db, $lis=null)
                {
                        $this->db = $db;
                        $this->team = new OwnTeamInLeagueInSeason();
                ...
                function init($lis)
                {
                        ...
                        $query = sprintf("select count(teamID) as number
                                          from   lmOwnTeamInLeagueInSeason
                                          where  seasonID = '%s'
                                          and    sportID = '%s'
                                          and    leagueID = '%s'
                                         ", $this->season->getID(),
                        ...
                        $this->db->executeQuery($query);
                        $this->db->nextRow();
                        $this->otilisNumber = $this->db->getValue("number");
...
/*
                        $query = sprintf("select    t.ID,
                                          ...
                                          left join lmOwnTeamInLeagueInSeason otilis
                                          using     (teamID, seasonID, sportID, leagueID)
                                          left join lmTeam t
                                          on        t.ID = tilis.teamID
...
*/
                        ...
                        while($this->db->nextRow())
                                $this->list[] = array( $this->db->getValue("ID"),
                                                       ...
                                                       $this->db->getValue("ownName") );
                        $this->number = sizeof($this->list);
                }

                function modifyList($ownteams)
                {
                        // delete all entries
                        $query = sprintf("delete
                                          from   lmOwnTeamInLeagueInSeason
                        ...
                        // add new entries from list
                        if(is_array($ownteams))
                        {
                                while(list($k, $v) = each($ownteams))
                                {
                                        if(!empty($v))
                                        {
                                                $query = sprintf("insert
                                                                into   lmOwnTeamInLeagueInSeason
                                                                        ( teamID,
                                                                        seasonID,
                                                                        sportID,
                                                                        leagueID,
                                                                ...
                                                                values ( '%s',
                                                                        ...
                                                                        '%s',
                                                                        '%s'  )
                                                                ", $k, $this->season->getID(), $this->sport->getID(), $this->league->getID(), $v );
                                                $this->db->executeQuery($query);
                                        }
                                }
                                return true;
                ...
                function next()
                {
                        ...
                        $this->team->isOwn = $this->list[$this->index][2];
                        $this->team->ownName = $this->list[$this->index][3];
                        $this->index++;
                        return $this->team;
                }
                ...
                function getNumberOfOwnTeamsInLeagueInSeason()
                {
                        return $this->otilisNumber;
                }
