<?php
    ...
    if(isset($go))
    {
        if( $go == "psuf" )
        {
            printEditForm($db, "Berechtigung Terminverwaltung ändern", $username);
        }
        ...
        else if ($go == "pef")
        {
            ...
            $query = sprintf("delete from authUserDate where username='%s'", $username);
            $db->executeQuery($query);

            // set new permissions
            $query = sprintf("insert into authUserDate (username, createPerm, deletePerm, modifyPerm) values('%s','%s','%s','%s')", $username, !empty($createperm)&&($createperm=="yes")?"yes":"no", !empty($deleteperm)&&($deleteperm=="yes")?"yes":"no", !empty($modifyperm)&&($modifyperm=="yes")?"yes":"no");
            $db->executeQuery($query);
    ...
    {
        printSelectUserForm($db, "", "Berechtigung Terminverwaltung ändern", "yes");
    }
    ...
    function printEditForm($db, $header="", $username="", $message="")
    {
        ...
        $query = sprintf("select createPerm, deletePerm, modifyPerm from authUserDate where username='%s'", $username);
        $db->executeQuery($query);
        $db->nextRow();
        print ("              <tr>\n");
