<?php
    ...
    if(isset($go))
    {
        if( $go == "psuf" )
        {
            printEditForm($db, "Berechtigung Spieleverwaltung ändern", $username);
        }
        ...
        else if ($go == "pef")
        {
            ...
            $query = sprintf("delete from authUserOthermatch where username='%s'", $username);
            $db->executeQuery($query);

            // set new permissions
            $query = sprintf("insert into authUserOthermatch (username, createPerm, deletePerm, modifyPerm, resultPerm) values('%s','%s','%s','%s','%s')", $username, !empty($createperm)&&($createperm=="yes")?"yes":"no", !empty($deleteperm)&&($deleteperm=="yes")?"yes":"no", !empty($modifyperm)&&($modifyperm=="yes")?"yes":"no", !empty($resultperm)&&($resultperm=="yes")?"yes":"no");
            $db->executeQuery($query);
    ...
    {
        printSelectUserForm($db, "", "Berechtigung Spieleverwaltung ändern", "yes");
    }
    ...
    function printEditForm($db, $header="", $username="", $message="")
    {
        ...
        $query = sprintf("select createPerm, deletePerm, modifyPerm, resultPerm from authUserOthermatch where username='%s'", $username);
        $db->executeQuery($query);
        $db->nextRow();
        print ("              <tr>\n");
        print ("                <td>eintragen</td>\n");
        printf("                <td><input type=\"checkbox\" name=\"createperm\" value=\"yes\" %s></input></td>", $db->getValue("createPerm")=="yes"?"checked":"");
        print ("              </tr>");
        print ("              <tr>\n");
        print ("                <td>ändern</td>\n");
        ...
        print ("              </tr>");
        print ("              <tr>\n");
        print ("                <td>Ergebnisse</td>\n");
        printf("                <td><input type=\"checkbox\" name=\"resultperm\" value=\"yes\" %s></input></td>", $db->getValue("resultPerm")=="yes"?"checked":"");
        print ("              </tr>");
